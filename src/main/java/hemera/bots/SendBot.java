    ...
    public Flowable<CommandsAndPendingSet> process(LedgerViewFlowable.LedgerView<Record> ledgerView) {
        ...
        List<SignedTransaction.Contract> signedAndPendingTransactions =
                signedTransactions.stream().filter(c -> c.data.sendStatus instanceof Pending)
                        .collect(Collectors.toList());
        List<SignedTrasferTransaction.Contract> signedTransferTransactions =
                (List<SignedTrasferTransaction.Contract>)(List<?>)
                        getContracts(ledgerView, SignedTrasferTransaction.TEMPLATE_ID);
        List<SignedTrasferTransaction.Contract> signedAndPendingTransferTransactions =
        ...
        pending.putIfAbsent(SignedTrasferTransaction.TEMPLATE_ID, new HashSet<>());
        ...
        List<Command> transferCommandList = signedAndPendingTransferTransactions.stream().map(contract -> {
            pending.get(SignedTrasferTransaction.TEMPLATE_ID).add(contract.id.contractId);
            try {
                EthSendTransaction resp = Web3jProvider.getInstance()
                        .web3j.ethSendRawTransaction(contract.data.signedTx).send();
                String txHash = resp.getTransactionHash();
                SendStatus sendStatus = new Sent(
                        ...
                        Web3jProvider.getInstance().version,
                        Optional.of(txHash));
                return contract.id.exerciseSignedTrasferTransaction_Sent(sendStatus);
            } catch (Exception e) {
                ...
                log.error(reason);
                return contract.id.exerciseSignedTrasferTransaction_Fail(reason);
            }
        }).collect(Collectors.toList());
